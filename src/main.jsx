...
const App = () => {
  ...
  const renderDraftContent = (data) => {
    const scale = 2.5; 
    const svgW = data.w * scale; 
    const svgD = data.d * scale; 
    const pad = 40;
    return (
  ...
  const addMass = (params = {}) => {
    ...
    const meshMat = new THREE.MeshPhysicalMaterial({ 
      color: 0xf8fafc, 
      transparent: true, 
      opacity: isGhostMode ? 0.2 : 1 
    });
    const mesh = new THREE.Mesh(geom, meshMat); 
    mesh.position.set(x, h/2, z);
    sceneRef.current.add(mesh); 
    massesRef.current.push({ id: mesh.uuid, mesh, w, h, d, material, program });
    showToast(`Block created: ${program}`);
  };
  ...
  const generateFromAI = async () => {
    ...
    try {
      ...
      showToast("AI Engine Sync Error"); 
    } finally { 
      setLoading(false); 
    }
  };
