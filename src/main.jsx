...
/**
 * SIMMORPH KERNEL v7.9.64 - PRODUCTION UNIFIED
 * 1. FIXED: Resolved Tailwind CDN malformed URL in index.html.
 * 2. FIXED: Added .js extensions to Three.js imports for Vite production build.
 * 3. FIXED: Refactored SVG transform logic to prevent esbuild syntax errors.
 */
...
const SimMorphApp = () => {
  ...
  useEffect(() => {
    ...
    tControls.addEventListener('dragging-changed', (e) => { if (controlsRef.current) controlsRef.current.enabled = !e.value; });
  ...
  return (
    <div className="relative h-screen w-screen bg-[#09090b] overflow-hidden font-sans text-slate-400 select-none text-left">
      ...
      <div className="absolute top-8 left-8 flex items-center gap-6 bg-[#1e1e20]/60 backdrop-blur-3xl p-5 rounded-full border border-white/5 shadow-2xl z-30">
        ...
        <span className="text-sm font-black uppercase text-white tracking-widest italic leading-none">SimMorph Kernel v7.9.64</span>
      </div>
      ...
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 w-[60vw] pointer-events-auto z-30">
        <div className="relative group">
          ...
          <div className="relative flex items-center bg-[#1e1e20]/98 backdrop-blur-[100px] border border-white/10 rounded-[4rem] pl-16 pr-6 py-6 shadow-2xl overflow-hidden">
            <input type="text" value={prompt} onChange={(e) => setPrompt(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && generateFromAI()} placeholder="Design intent kernel..." className="flex-1 bg-transparent text-white text-lg focus:outline-none placeholder:text-white/5 font-medium tracking-tight" />
...
export default SimMorphApp;import React, { useState, useEffect, useRef, useMemo, useCallback } from 'react';
import { createRoot } from 'react-dom/client';
import * as THREE from 'three';
import { OrbitControls } from 'three/examples/jsm/controls/OrbitControls.js';
import { TransformControls } from 'three/examples/jsm/controls/TransformControls.js';
...
try {
  ...
  console.warn("SimMorph: Cloud integration deferred.");
}

const MATERIALS = {
  concrete: { color: 0x94a3b8, label: 'Structural Concrete' },
  glass: { color: 0xbae6fd, transparent: true, opacity: 0.3, label: 'High-Perf Glass' },
  timber: { color: 0x92400e, label: 'CLT Panel' },
  default: { color: 0xf8fafc, label: 'Massing Shell' }
};
...
const SimMorphApp = () => {
  ...
  const addMass = useCallback((params = {}) => {
    if (!sceneRef.current) return;
    const { w = 50, h = 100, d = 50, x = 0, z = 0, material = 'default', program = 'General Zone' } = params;
    const geom = new THREE.BoxGeometry(w, h, d);
    const matData = MATERIALS[material] || MATERIALS.default;
    const meshMat = new THREE.MeshPhysicalMaterial({ color: matData.color, transparent: true, opacity: isGhostMode ? 0.2 : 1 });
    const mesh = new THREE.Mesh(geom, meshMat); mesh.position.set(x, h/2, z);
    sceneRef.current.add(mesh); massesRef.current.push({ id: mesh.uuid, mesh, w, h, d, material, program });
